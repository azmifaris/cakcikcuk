...
class Keuangan_rekap extends CI_Controller
{
	...
	public function list_ganjil(){
		$this->load->model("KeuanganRekap_model");
		$data["fetch_data"] = $this->KeuanganRekap_model->list_ganjil();
		$this->load->view("adminstba/keuangan/keuangan_rekap", $data);
	}
	...
	public function tambah_aksi(){
		...
		if ($tipe_bayar == 'SPP Variabel') {
 		...
 		if($jumlah_dibayarkan == $total){
			...
			$data2 = array(
			'spp_tetap' => $status);
			}else if ($tipe_bayar =='Kemahasiswaan') {
			$data2 = array(
			'kemahasiswaan' => $status);
 			}else if ($tipe_bayar == 'Wisuda') {
			$data2 = array(
			'wisuda' => $status);
			}else if ($tipe_bayar == 'Kesehatan') {
			$data2 = array(
			'kesehatan' => $status);
			};
		}else{
		...
		$data = array(
         ...
         echo "$tipe_bayar";   
          ...
          $result2 = $this->KeuanganRekap_model->update_status($where2, $data2,'tbl_keuangan_status');    
...
*/       	redirect('admin/keuangan/keuangan_rekap');
	...
	public function update(){
		...
		echo "$nim $status $izin $tipe_bayar";
	...
	public function reset_status($id=null){
		$this->load->model("KeuanganRekap_model");
	...
	public function detail_mahasiswa($nim){
		$this->load->model("KeuanganRekap_model");
		$data["detail_mahasiswa"] = $this->KeuanganRekap_model->detail_mahasiswa($nim);	
		$data["mahasiswa"] = $this->KeuanganRekap_model->mahasiswa_id($nim);
		$this->load->view("adminstba/keuangan/keuangan_mahasiswaDetail", $data);
	}
