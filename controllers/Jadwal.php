...
//        die();
	...
	function search(){
        ...
        if($mk!="" && $id_jurusan==""){
            $text_sql="and (b.id_jurusan='$id_jurusan' or a.nama_mk like '%$mk%')";
        }else if($mk=="" && $id_jurusan!=""){
            $text_sql="and (b.id_jurusan='$id_jurusan')";
        }else{
            $text_sql="";
        }
        $smt_aktif=$this->db->query("SELECT * FROM setup_aktif")->row();
        ...
        if($this->session->userdata('login_data')->status=='dosen'){
            $dosen=$this->db->query("select * from tbl_dosen where nidn='".$this->session->userdata('login_data')->nomor_induk."'")->row();
            $mk_daftar=$this->db->query("SELECT * FROM tb_matakuliah a 
            JOIN tb_jurusan b ON a.`id_jurusan`=b.`id_jurusan`
            JOIN tb_jenjang c ON b.`id_jenjang`=c.`id_jenjang`
            ...
            LEFT JOIN tbl_hari i ON i.`id_hari`=g.`id_hari`
            LEFT JOIN tb_ruangan j ON j.`id`=g.`id_ruang`
            WHERE ploting_id_dosen='$dosen->id_dosen' and status='1' and a.semester='$smt_aktif->smt' and a.tahun_ajaran='$smt_aktif->ta' and ploting_status='Y'  ".$text_sql."  order by i.id_hari,h.id_jam")->result();
        }else{
            $mk_daftar=$this->db->query("SELECT * FROM tb_matakuliah a 
            JOIN tb_jurusan b ON a.`id_jurusan`=b.`id_jurusan`
            JOIN tb_jenjang c ON b.`id_jenjang`=c.`id_jenjang`
            ...
            LEFT JOIN tbl_hari i ON i.`id_hari`=g.`id_hari`
            LEFT JOIN tb_ruangan j ON j.`id`=g.`id_ruang`
            WHERE status='1' and a.semester='$smt_aktif->smt' and a.tahun_ajaran='$smt_aktif->ta' and ploting_status='Y'  ".$text_sql."  order by id_jadwal_mk desc,nama_jenjang,nama_jurusan,nama_mk")->result();
        }
//        echo '<pre>';
//        var_dump($mk_daftar);
//        die();
